...
-(void) refreshReceipt {
    #if !MAC_APPSTORE
    if ([self isiOS6OrEarlier]) {
        Log(@"RefreshReceipt not supported on iOS < 7!");
        return;
    }
    #endif

    self.receiptRefresher = [[ReceiptRefresher alloc] initWithCallback:^(BOOL success) {
        Log(@"RefreshReceipt status %d", success);
        if (success) {
...
- (void)onTransactionSucceeded:(SKPaymentTransaction*)transaction {
    ...
    if (nil == transactionId) {
        ...
        Log(@"Missing transaction Identifier!");
    }
...
-(void) finishTransaction:(NSString *)transactionIdentifier {
    ...
    if (nil != transaction) {
        Log(@"Finishing transaction %@", transactionIdentifier);
        [[SKPaymentQueue defaultQueue] finishTransaction:transaction];
        [pendingTransactions removeObjectForKey:transactionIdentifier];
    } else {
        Log(@"Transaction %@ not found!", transactionIdentifier);
    }
}
...
{
    productIds = paramIds;
    Log(@"RequestProducts:%@", productIds);
...
-(void) purchaseProduct:(ProductDefinition*)productDef
{
    ...
    if (requestedProduct != nil) {
        Log(@"PurchaseProduct: %@", requestedProduct.productIdentifier);
        
        if ([SKPaymentQueue canMakePayments]) {
            SKPayment *paymentRequest = [SKPayment paymentWithProduct:requestedProduct];
            [[SKPaymentQueue defaultQueue] addPayment:paymentRequest];
        } else {
            Log(@"PurchaseProduct: IAP Disabled");
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response {
    ...
    Log(@"ProductsRequest:didReceiveResponse:%@", response.products);
    // Record our products.
...
- (void)request:(SKRequest *)request didFailWithError:(NSError *)error {
    ...
    Log(@"SKProductRequest::didFailWithError: %ld, %@. Unity Purchasing will retry in %i seconds", (long)error.code, error.description, delayInSeconds);
...
{
    Log(@"restoreCompletedTransactionsFailedWithError");
...
+ (NSString*) serializeProductMetadata:(NSArray*)appleProducts
{
    ...
    for (id product in appleProducts) {
        if (NULL == [product productIdentifier]) {
            Log(@"Product is missing an identifier!");
            continue;
        }
        ...
        if (NULL == formattedString) {
            Log(@"Unable to format a localized price");
        ...
        if (NULL == [product localizedTitle]) {
            Log(@"No localized title for: %@. Have your products been disapproved in itunes connect?", [product productIdentifier]);
        ...
        if (NULL == [product localizedDescription]) {
            Log(@"No localized description for: %@. Have your products been disapproved in itunes connect?", [product productIdentifier]);
...
UnityPurchasing* _instance = NULL;

UnityPurchasing* _getInstance() {
    if (NULL == _instance) {
        _instance = [[UnityPurchasing alloc] init];
    }
    return _instance;
}
...
void unityPurchasingRetrieveProducts(const char* json) {
    ...
    [_getInstance() requestProducts:productIds];
}
...
void unityPurchasingPurchase(const char* json, const char* developerPayload) {
    ...
    [_getInstance() purchaseProduct:product];
}
...
void unityPurchasingFinishTransaction(const char* productJSON, const char* transactionId) {
    ...
    [_getInstance() finishTransaction:tranId];
}
